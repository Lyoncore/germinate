        ...
        for pkg in self.packages.keys():
            for field in "Pre-Depends", "Depends", "Recommends", "Suggests":
                for deplist in self.packages[pkg][field]:
                    for dep in deplist:
                        if dep[0] in self.packages:
                            self.packages[dep[0]]["Reverse-Depends"].append(
                                pkg + " (" + field + ")")
...
def write_rdepend_list(filename, g):
    pkg_len = len("Package")
    dep_len = len("Reverse Dependencies")
    ...
    f = open(filename, "w")
    print >>f, "%-*s | %-*s" % (pkg_len, "Package",
                                dep_len, "Reverse Dependencies")
    print >>f, ("-" * pkg_len) + "-+-" + ("-" * dep_len) + "-"
    for pkg in list:
        dlist = g.packages[pkg]["Reverse-Depends"]
        dlist.sort()
        for seed in g.seeds:
            if pkg in g.seed[seed]:
                dlist.insert(0, seed.title() + " seed")

        print >>f, "%-*s | %-*s" % (pkg_len, pkg, dep_len, ", ".join(dlist))
